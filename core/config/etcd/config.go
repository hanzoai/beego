...
func (provider *EtcdConfigerProvider) ParseData(data []byte) (config.Configer, error) {
	...
	// etcd client without grpc-prometheus interceptors — the hanzo
	// fork tracks etcd telemetry through luxfi/metric directly via the
	// caller's instrumentation, not via grpc-ecosystem/go-grpc-prometheus.
